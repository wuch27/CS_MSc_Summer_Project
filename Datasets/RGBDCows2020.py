...
class RGBDCows2020(data.Dataset):
	...
	@staticmethod
	def displayBboxes(dataset_dir):
		# Construct the dataset directories
		rgb_path = os.path.join(dataset_dir, "RGB")
		dep_path = os.path.join(dataset_dir, "Depth")
		lab_path = os.path.join(dataset_dir, "Labels")

		# Find all the label files
		labels = DataUtils.allFilesAtDirWithExt(lab_path, ".xml")

		# Iterate over each one
		for label in tqdm(labels):
			# Base filename (e.g. 00023)
			base_filename = os.path.basename(label)[:-4]

			# Load the corresponding rgb and depth images
			rgb = cv2.imread(os.path.join(rgb_path, base_filename+".jpg"))
			dep = cv2.imread(os.path.join(dep_path, base_filename+".jpg"))

			# Load the annotation
			annotations = DataUtils.readRotatedXMLAnnotation(label)

			# Render these rotated rectangles
			vis_rgb = VisualisationManager.drawRotatedBbox(rgb, annotations)
			vis_dep = VisualisationManager.drawRotatedBbox(dep, annotations)

			# Display them side by side
			cv2.imshow("Visualisation", np.concatenate((vis_rgb, vis_dep), axis=1))
			cv2.waitKey(0)
